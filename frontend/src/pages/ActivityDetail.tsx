...
const ActivityDetail: React.FC = () => {
  ...
  const [commentLikeLoading, setCommentLikeLoading] = useState<string | null>(
    null
  );
  ...
  const checkRegistrationStatus = async () => {
    ...
    try {
      ...
      console.log(
        '✅ [DEBUG] checkRegistrationStatus - 请求成功:',
        response.data
      );
      ...
      console.error(
        '❌ [DEBUG] checkRegistrationStatus - 错误详情:',
        error.response?.data
      );
      setIsRegistered(false);
    }
        ...
        comments.map(async comment => {
          try {
            const response = await api.get(
              `/comments/${comment.id}/like-status`
            );
            return {
              ...comment,
              isLiked: response.data.data.liked,
  ...
  const fetchActivityDetail = async (shouldUpdateView = true) => {
    ...
    try {
      ...
      if (activityData.comments && activityData.comments.length > 0) {
        const commentsWithLikeStatus = await checkCommentLikeStatus(
      ...
      if (shouldUpdateView && !viewCountUpdated) {
        ...
        console.log(
          '⏭️ [DEBUG] 跳过浏览量更新 (shouldUpdateView:',
          shouldUpdateView,
          ', viewCountUpdated:',
          viewCountUpdated,
          ')'
  ...
  const handleCancelRegistration = async () => {
    try {
      ...
      console.error(
        '❌ [DEBUG] handleCancelRegistration - 取消报名失败:',
        error
      );
  ...
  const handleCommentSubmit = async (values: any) => {
    try {
      ...
      console.error(
        '❌ [DEBUG] handleCommentSubmit - 错误详情:',
        error.response?.data
      );
  ...
  const handleDeleteActivity = async () => {
    ...
    if (activity.organizer.id === user.id) {
      try {
        // 调用删除API
        await api.delete(
          `/activities/delete-by-name/${encodeURIComponent(activity.title)}`
        );
    ...
    <div className='activity-detail' style={{ padding: '24px' }}>
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col xs={24} lg={16}>
          ...
          <Card>
            <div style={{ marginBottom: '16px' }}>
              <Space>
                {activity.isRecommended && <Tag color='red'>推荐</Tag>}
            ...
            {(() => {
              console.log(
                `[活动详情封面调试] 活动ID: ${activity.id}, 标题: ${activity.title}`
              );
              console.log(
                `[活动详情封面调试] coverImage: ${activity.coverImage || '未设置'}`
              );
              ...
              if (activity.coverImage) {
                ...
                console.log(
                  `[活动详情封面调试] 添加封面图片: ${activity.coverImage}`
                );
              }
              if (activity.images) {
                // 添加其他图片，但排除已经作为封面的图片
                        ...
                        <Col key={index} xs={12} sm={8} md={6}>
                          <Image
                            ...
                            style={{
                              ...
                              borderRadius: 8,
                              border:
                                index === 0 && activity.coverImage
                                  ? '2px solid #1890ff'
                                  : 'none',
                            }}
                            onLoad={() =>
                              console.log(
                                `[活动详情封面调试] 图片加载成功: ${image}`
                              )
                            }
                            onError={e => {
                              console.error(
                                `[活动详情封面调试] 图片加载失败: ${image}`,
                                e
                              );
                            }}
                          />
          ...
          <Card
            ...
            extra={
              <Button type='primary' onClick={() => setShowCommentModal(true)}>
                写评价
              </Button>
            }
          >
        ...
        <Col xs={24} lg={8}>
            ...
            <Descriptions column={1} size='small'>
              ...
              <Descriptions.Item label='报名截止'>
                <Text>
                  ...
                  {dayjs(activity.registrationDeadline).format(
                    'YYYY-MM-DD HH:mm'
                  )}
                </Text>
              </Descriptions.Item>
            ...
            <Space direction='vertical' style={{ width: '100%' }} size='middle'>
              ...
              >
                {(() => {
                  ...
                  const now = dayjs();
                  const registrationDeadline = dayjs(
          ...
          <Card title='组织者' style={{ marginTop: '16px' }}>
            ...
            >
              ...
              >
                {activity.organizer.nickname?.[0] ||
                  activity.organizer.username[0]}
              </Avatar>
              <div>
            ...
            <Space direction='vertical' style={{ width: '100%' }}>
              ...
              {(() => {
                ...
                const isBeforeStart = dayjs().isBefore(
                  dayjs(activity.startTime)
                );
                const shouldShowButton =
                  isUserLoggedIn && isCreator && isBeforeStart;
                ...
                console.log(
                  '  - 活动开始时间:',
                  dayjs(activity.startTime).format('YYYY-MM-DD HH:mm:ss')
                );
          ...
          <Form.Item
            ...
            rules={[
              { required: true, message: '请选择参与人数' },
              {
                ...
                message: `人数范围：1-${getRemainingSlots()}`,
              },
            ]}
          >
            <InputNumber
      ...
      >
        ...
        >
          ...
          <Form.Item>
            <Space style={{ width: '100%', justifyContent: 'flex-end' }}>
              <Button onClick={() => setShowCommentModal(false)}>取消</Button>
